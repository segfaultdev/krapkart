// DONE: Fix other's job :D

window.iJava2Javascript = function(tree) {
  var deep = 0;
  var rContext = [];
  
  this.doIt = function() {
    console.log("Don't worry user, you're now in good hands!");
  ...
  var translate = function(node, context) {
    ...
    if (node.type === "value") {
      ...
      if (node.id === "{...}") {
        ...
        var s = showArrayInitializer(node.right, context);
        return s;
      } else 
      if (node.id === "(cast)") {
        var s = "";
      ...
      if (node.id === ".") {
        ...
        if (!done) {
          ...
          if (node.datatype == IntegerDatatype) {
            ...
            if ( node.id === "/=" || node.id === "%=") {
              // Convierto la autooperaci�n en una operaci�n normal
              node.id = node.id.charAt(0);
              closeParenthesis = true;
              // A�ado la parte de asignaci�n
...
//          if (node.yes.id !== "(block)") s = s + " ; ";
  ...
  function get_ndim_array(dims, value) {
    if (dims.length == 0) {
      return value;
    }
    
    return "Array(" + dims[0] + ").fill(" + get_ndim_array(dims.slice(1), value) + ")";
  }
  ...
  function operadorNew(node, context) {
    ...
    if (node.datatype instanceof ArrayDatatype) {
      ...
      for (let i = 0; i < node.args.length; i++) {
        dims.push(translate(node.args[i], context));
      }
      
      s = get_ndim_array(dims, node.datatype.celltype.getDefaultInitializer());
    } else {
      s = s + "(new ";
